...
const val YEAR_MONTH_FORMAT_PATTERN = "MMMM yyyy"
...
fun YearMonth.toFormattedStringOrNull(pattern: String = YEAR_MONTH_FORMAT_PATTERN): String? {
    try {
        return toFormattedStringOrThrowError(pattern = pattern)
    } catch (e: Throwable) {
        AppLogger.e(e)
        return null
    }
...
fun YearMonth.toFormattedStringOrThrowError(pattern: String = YEAR_MONTH_FORMAT_PATTERN): String {
    val formatter = DateTimeFormatter.ofPattern(pattern)
    return formatter.format(this)
}